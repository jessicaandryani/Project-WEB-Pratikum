...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 pb-4 border-b border-gray-200">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Daftar Pengguna</h1>
            <p class="mt-1 text-sm text-gray-500">Kelola semua akun pengguna yang terdaftar di sistem.</p>
        </div>
        <div class="mt-4 sm:mt-0 flex space-x-3">
            <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
    ...
    @include('partials.admin.flash_messages') {{-- Untuk menampilkan session success/error --}}
        ...
        <form method="GET" action="{{ route('admin.users.index') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-end">
            ...
            <div>
                ...
                <select name="role" id="role" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    ...
                    <option value="user" {{ request('role') == 'user' ? 'selected' : '' }}>User (Tamu)</option>
                </select>
            </div>
            <div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <span class="inline-flex items-center justify-center h-10 w-10 rounded-full bg-indigo-100 text-indigo-700 font-semibold">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{{ ucfirst($user->role) }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $user->created_at->isoFormat('D MMM YYYY, HH:mm') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('admin.users.edit', $user->id) }}" class="text-yellow-500 hover:text-yellow-700 p-1 rounded-md hover:bg-yellow-50" title="Edit Pengguna">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg>
                                    </a>
                                    @if(Auth::id() !== $user->id) {{-- Admin tidak bisa hapus diri sendiri --}}
                                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengguna ini? Aksi ini tidak dapat diurungkan.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:text-red-700 p-1 rounded-md hover:bg-red-50" title="Hapus Pengguna">
    ...
    @else
        <div class="text-center py-12 bg-white shadow-lg sm:rounded-lg">
            <svg class="mx-auto h-12 w-12 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.182 16.318A4.5 4.5 0 0018 17.25a4.5 4.5 0 00-3.818-4.405M12 12a4.5 4.5 0 011.682-1.095M15.182 16.318A4.5 4.5 0 0112 17.25a4.5 4.5 0 01-3.182-.932m3.182-1.095A4.5 4.5 0 0012 10.75a4.5 4.5 0 00-3.182 4.471m5.364 0H6.82A4.5 4.5 0 003 16.318m3.818-4.405A4.5 4.5 0 0112 10.75a4.5 4.5 0 013.182 1.163M15.182 16.318a4.5 4.5 0 01.529 .111M12 3.75a.75.75 0 01.75.75v2.25a.75.75 0 01-1.5 0V4.5a.75.75 0 01.75-.75zM12 18.75a.75.75 0 01.75.75V21.75a.75.75 0 01-1.5 0v-2.25a.75.75 0 01.75-.75zM4.5 12a.75.75 0 01.75-.75h2.25a.75.75 0 010 1.5H5.25a.75.75 0 01-.75-.75zM18.75 12a.75.75 0 01.75-.75h2.25a.75.75 0 010 1.5h-2.25a.75.75 0 01-.75-.75z" />
            </svg>
            <h3 class="mt-2 text-xl font-medium text-gray-900">Tidak Ada Pengguna</h3>
            <p class="mt-1 text-sm text-gray-500">Tidak ada data pengguna yang cocok dengan filter Anda, atau belum ada pengguna terdaftar.</p>
        </div>
    @endif
</div>
@endsection
