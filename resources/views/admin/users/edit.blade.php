...
<div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="bg-white shadow-xl rounded-lg p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="address" id="address" rows="3"
                      class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('address') border-red-500 @enderror">{{ old('address', $user->address) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-200 mt-6">
            <div>
                ...
                <select name="role" id="role" required
                    ...
                    <option value="user" {{ old('role', $user->role) == 'user' ? 'selected' : '' }}>User (Tamu)</option>
                ...
                <label class="block text-sm font-medium text-gray-700">Status Akun <span class="text-red-500">*</span></label>
                 <div class="mt-2 space-y-2">
                    <div class="flex items-center">
                        <input id="is_active_true" name="is_active" type="radio" value="1" {{ old('is_active', $user->is_active) == '1' ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                        <label for="is_active_true" class="ml-3 block text-sm font-medium text-gray-700">Aktif</label>
                    </div>
                    <div class="flex items-center">
                        <input id="is_active_false" name="is_active" type="radio" value="0" {{ old('is_active', $user->is_active) == '0' ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
